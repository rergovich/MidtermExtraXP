...
*/
/*
let runTime = parseInt(prompt('How many times would you like to run the program?'))

while (let i <= runTime) {
  let dogType = prompt('What size is your poodle: standard, miniature, toy').toLowerCase
  if (dogType == 'toy') {
  alert(`${dogType} dogs are so tiny.`)
} else if (dogType == 'miniature') {
  alert(`${dogType} dogs are very small but hardy.`)
} else {
  alert(`${dogType} dogs are really very large.`)
}
}
*/